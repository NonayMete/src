...
public:
	...
	double LT, RT, RX, RY, LX, LY, speedL, speedR, rate, dist, offset, accel,
			velocity, center, t; bool LB, RB, back, start, A, B, X, Y, up, down,
			left, right, IR, IR2;
	...
	void RobotInit() {
		...
		AutoChooser->AddObject("Nothing", (int*) 4);
...
//}
	...
	void AutonomousPeriodic() {
		getValues();
		updateDash();
		if ((dist > 100) && (center > 240 && center < 400)) {
			auto_state = "straight";
			tankDrive(-0.5, -0.53);
		}
		if ((dist > 65) && (center > 240 && center < 400)) {
			auto_state = "slow";
			tankDrive(-0.4, -0.43);
		}
		...
		if(dist == 1234567890){
			if(mult == -1) auto_state = "Error - Going Right";
			if(mult == 1) auto_state = "Error - Going Left";
